...
public final class TypeResolverHelper {
	...
	private static boolean isStaticCandidate(Method method, Class<?> factoryClass) {
	...
	public static ResolvableType getFunctionArgumentType(ResolvableType functionType, int argumentIndex) {
		...
		if (Function.class.isAssignableFrom(resolvableClass)) {
			...
			if (KotlinDelegate.isKotlinSupplier(resolvableClass)) {
				functionArgumentResolvableType = KotlinDelegate.adaptToKotlinSupplierType(functionType);
			}
			else if (KotlinDelegate.isKotlinFunction(resolvableClass)) {
		...
		if (functionArgumentResolvableType == ResolvableType.NONE) {
			throw new IllegalArgumentException("Unresolvable type [" + functionType
					+ "]. Expected java.util.function.[Function|BiFunction|Supplier|Consumer]");
		}

		if (argumentIndex > functionArgumentResolvableType.getGenerics().length - 1) {
			throw new IllegalArgumentException("Invalid argumentIndex [" + argumentIndex + "] for type [" + functionType
					+ "] with [" + functionArgumentResolvableType.getGenerics().length + "] arguments");
		}

		return (functionArgumentResolvableType.getGeneric(argumentIndex) != ResolvableType.NONE)
				? functionArgumentResolvableType.getGeneric(argumentIndex) : ResolvableType.forClass(Object.class);
	}
