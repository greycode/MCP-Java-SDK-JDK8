...
import com.mycompany.aigw.spec.McpSchema.CallToolResult;
...
public class SyncMcpToolCallback implements ToolCallback {
	...
	public ToolDefinition getToolDefinition() {
		return DefaultToolDefinition.builder()
			.name(McpToolUtils.prefixedToolName(this.mcpClient.getClientInfo().getName(), this.tool.getName()))
	...
	public String call(String functionInput) {
		...
		// getToolDefinition
		CallToolResult response = this.mcpClient.callTool(new CallToolRequest(this.tool.getName(), arguments));
		if (response.getIsError() != null && response.getIsError()) {
			throw new IllegalStateException("Error calling tool: " + response.getContent());
		}
		return ModelOptionsUtils.toJsonString(response.getContent());
	}
