...
public class WebFluxSseClientTransport implements McpClientTransport { //
	 ...
	 * Anonymous inner class syntax is JDK 8 compatible.
	 */
	 ...
	 * Sinks API exists in Reactor versions compatible with Spring Boot 2.x.
	 */
	...
	public WebFluxSseClientTransport(WebClient.Builder webClientBuilder, ObjectMapper objectMapper,
									 String sseEndpoint) { //
		// Assuming io.modelcontextprotocol.util.Assert is compatible or replaced.
	...
	public Mono<Void> connect(Function<Mono<JSONRPCMessage>, Mono<JSONRPCMessage>> handler) { //
		...
		// Lambda expressions and Reactor operators (concatMap, handle, transform) are JDK 8+ compatible.
	...
	public Mono<Void> sendMessage(JSONRPCMessage message) { //
		...
		return messageEndpointSink.asMono().flatMap(messageEndpointUri -> { //
			...
			try {
				...
				// WebClient POST request, compatible if WebFlux is used.
	...
	protected Flux<ServerSentEvent<String>> eventStream() {// @formatter:off
		// WebClient GET request for SSE stream.
		return this.webClient //
				...
				.retrieve() //
				.bodyToFlux(SSE_TYPE) //
				// Retry operator exists in compatible Reactor versions.
	 ...
	 * BiConsumer lambda is JDK 8 compatible.
	 */
	private BiConsumer<RetrySignal, SynchronousSink<Object>> inboundRetryHandler = (retrySpec, sink) -> { //
		...
		// instanceof check is standard Java.
	...
	public Mono<Void> closeGracefully() { // @formatter:off
		// Schedulers and Reactor disposal logic are compatible.
	...
	public <T> T unmarshalFrom(Object data, TypeReference<T> typeRef) { //
		// Jackson's convertValue is standard.
