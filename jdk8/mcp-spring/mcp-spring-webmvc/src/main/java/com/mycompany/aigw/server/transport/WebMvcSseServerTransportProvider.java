...
public class WebMvcSseServerTransportProvider implements McpServerTransportProvider { //
	...
	public WebMvcSseServerTransportProvider(ObjectMapper objectMapper, String baseUrl, String messageEndpoint,
		...
		// RouterFunctions.route() is compatible with Spring Framework 5 / Boot 2.x
	...
	public Mono<Void> notifyClients(String method, Object params) { //
		...
		// Flux and Mono usage is compatible with Spring Boot 2.x (with Reactor Core)
	...
	private ServerResponse handleSseConnection(ServerRequest request) { //
		...
		try {
			// ServerResponse.sse is compatible with Spring Framework 5 / Boot 2.x
			return ServerResponse.sse(sseBuilder -> { //
				...
				try {
					...
					// sseBuilder.error(e); // Removed as it might prevent graceful handling below
					// Rethrow or handle differently if needed, or ensure cleanup happens
					// For simplicity here, we rely on outer catch block or session removal logic
					// A better approach might involve closing the session explicitly here.
				}
			}, Duration.ZERO); // java.time.Duration is JDK 8+ //
		}
		catch (Exception e) { //
			logger.error("Failed to establish SSE connection for session {}: {}", sessionId, e.getMessage()); //
			sessions.remove(sessionId); // Ensure cleanup if session was added before exception //
	...
	private ServerResponse handleMessage(ServerRequest request) { //
		...
		// request.param() is available in Spring Framework 5 / Boot 2.x ServerRequest
		if (!request.param("sessionId").isPresent()) { //
			// Returning McpError directly assumes it can be serialized appropriately
			// Might need configuration with HttpMessageConverters or return a string/map
		...
		try {
			// request.body(String.class) requires appropriate HttpMessageReader
			// In Spring Boot 2.x with spring-boot-starter-web, StringMessageConverter is usually registered
			String body = request.body(String.class); //
			McpSchema.JSONRPCMessage message = McpSchema.deserializeJsonRpcMessage(objectMapper, body); //

			// Process the message through the session's handle method
			// .block() bridges reactive Mono<Void> to synchronous processing expected in WebMVC handler signature
			session.handle(message).block(); // Use with caution in Servlet threads //

			return ServerResponse.ok().build(); //
		}
		catch (IllegalArgumentException | IOException e) { // Multi-catch is JDK 7+ //
	...
	private class WebMvcMcpSessionTransport implements McpServerTransport { //
		...
		public Mono<Void> sendMessage(McpSchema.JSONRPCMessage message) { //
			// Using Mono.fromRunnable to wrap synchronous SSE send operation
			return Mono.fromRunnable(() -> { //
				...
				catch (Exception e) { // Catching broad Exception, consider more specific ones if possible //
					logger.error("Failed to send message to session {}: {}", sessionId, e.getMessage()); //
					// sseBuilder.error(e); // Propagating error might close the stream prematurely
					// Consider just logging or implementing custom error handling/retry
				}
			});
		}
		...
		public <T> T unmarshalFrom(Object data, TypeReference<T> typeRef) { //
			// objectMapper.convertValue is standard Jackson usage
