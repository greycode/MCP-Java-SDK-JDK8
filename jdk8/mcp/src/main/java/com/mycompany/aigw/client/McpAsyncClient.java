...
import com.mycompany.aigw.spec.McpClientSession.NotificationHandler;
import com.mycompany.aigw.spec.McpClientSession.RequestHandler;
...
public class McpAsyncClient {
	...
	McpAsyncClient(McpClientTransport transport, Duration requestTimeout, Duration initializationTimeout,
		...
		this.protocolVersions = Collections.singletonList(McpSchema.LATEST_PROTOCOL_VERSION);
	...
	private <T> Mono<T> withInitializationCheck(String actionName,
		...
		return this.initializedSink.asMono().timeout(this.initializationTimeout)
	...
	private RequestHandler<McpSchema.ListRootsResult> rootsListRequestHandler() {
		return params -> {
			...
			List<Root> rootsList = new ArrayList<>(this.roots.values());

			return Mono.just(new McpSchema.ListRootsResult(rootsList));
		};
	}
	...
	private NotificationHandler asyncResourcesChangeNotificationHandler(
		...
		return params -> listResources().flatMap(listResourcesResult -> Flux.fromIterable(resourcesChangeConsumers)
				.flatMap(consumer -> consumer.apply(listResourcesResult.getResources())).onErrorResume(error -> {
	...
	public Mono<McpSchema.CompleteResult> completeCompletion(McpSchema.CompleteRequest completeRequest) {
		return this.withInitializationCheck("complete completions",
				initializedResult -> this.mcpSession.sendRequest(McpSchema.METHOD_COMPLETION_COMPLETE, completeRequest,
