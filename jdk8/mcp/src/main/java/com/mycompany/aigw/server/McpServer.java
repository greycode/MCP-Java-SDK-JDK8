...
import com.mycompany.aigw.spec.McpSchema.ResourceTemplate;
import com.mycompany.aigw.spec.McpServerTransportProvider;
import com.mycompany.aigw.util.Assert;
import com.mycompany.aigw.util.DefaultMcpUriTemplateManagerFactory;
...
public interface McpServer {
	...
	class SyncSpecification {
		...
		public SyncSpecification rootsChangeHandlers(
			...
			Assert.notNull(handlers, "Handlers list must not be null");
			List<BiConsumer<McpSyncServerExchange, List<McpSchema.Root>>> handlersList = Arrays.asList(handlers);
			return this.rootsChangeHandlers(handlersList);
		}
		...
		public McpSyncServer build() {
			...
			McpAsyncServer asyncServer = new McpAsyncServer(this.transportProvider, mapper, asyncFeatures,
					this.requestTimeout, this.uriTemplateManagerFactory);
